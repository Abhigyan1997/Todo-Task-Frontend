...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchTasks = async () => {
            try {
                const response = await axios.get('http://13.233.158.103/api/todos', {
                    headers: {
    ...
    const saveTask = async (task) => {
        try {
            if (currentTask) {
                await axios.put(`http://13.233.158.103/api/todos/${currentTask._id}`, task, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
                    },
                });
                ...
                const response = await axios.post('http://13.233.158.103/api/todos', { ...task, status: 'TODO', createdAt: new Date().toISOString() }, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('token')}`,
                    },
                });
    ...
    const deleteTask = async (id) => {
        try {
            await axios.delete(`http://13.233.158.103/api/todos/${id}`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
                },
            });
    ...
    const moveTask = async (taskId, targetStatus) => {
        ...
        try {
            await axios.put(`http://13.233.158.103/api/todos/${taskId}`, { status: targetStatus }, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
                },
            });
    ...
    const handleLogout = async () => {
        try {
            const response = await axios.get('http://13.233.158.103/api/logout', {
